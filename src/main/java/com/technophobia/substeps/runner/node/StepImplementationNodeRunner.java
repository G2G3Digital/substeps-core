...
public class StepImplementationNodeRunner extends AbstractNodeRunner<StepImplementationNode, Void> {
    ...
    protected boolean execute(StepImplementationNode node, NodeExecutionContext context) {
        ...
        try {
            ...
            context.getMethodExecutor().executeMethod(node.getTargetClass(), node.getTargetMethod(), node.getMethodArgs());
    ...
    private <T> byte[] attemptScreenshot(StepImplementationNode node, NodeExecutionContext context) {
        ...
        try {
            log.info(Boolean.valueOf(node.getTargetClass().newInstance() instanceof ProvidesScreenshot).toString());
        ...
        log.info(node.getTargetClass() + " is Assignable? " + ProvidesScreenshot.class.isAssignableFrom(node.getTargetClass()));

        return ProvidesScreenshot.class.isAssignableFrom(node.getTargetClass()) ? getScreenshot(context, (Class<? extends ProvidesScreenshot>) node.getTargetClass())
                : null;
    }
